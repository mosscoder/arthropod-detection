...
- Test data: Creates symlinks to original images (no tiling)
...
- data/yolo_dataset/test/images/ - Test image symlinks
...
def save_tile_image(source_image_path, tile, output_path, scale_factor):
    """Extract and save tile from resized image."""
    with Image.open(source_image_path) as img:
        ...
        # Crop the tile region from resized image
        tile_img = resized_img.crop((
            tile['x'],
            tile['y'],
            tile['x'] + tile['width'],
            tile['y'] + tile['height']
        ...
        # Save the tile
        tile_img.save(output_path, quality=95)
...
def process_single_image_worker(args):
    ...
    for tile_data in tiles_data:
        ...
        if os.path.exists(tile_image_path):
            ...
            # Save tile image from resized source
            save_tile_image(image_path, tile, tile_image_path, tile_data['scale_factor'])
...
def create_dataset_files(image_groups, image_list, split_name, class_to_idx, dataset_dir, tile_mapping, use_tiling=True, overlap_ratio=0.1):
    """Generate tile images and YOLO format labels using multiprocessing, or symlinks for test."""
    ...
    if use_tiling:
        ...
        print(f"🔗 Creating symlinks for {split_name} split...")
        
        # Create subdirectories for this split
        images_dir = os.path.join(dataset_dir, split_name, 'images')
        labels_dir = os.path.join(dataset_dir, split_name, 'labels')
        
        # Create symlinks to original images and generate labels
        return create_symlinks_for_split(image_groups, image_list, split_name, class_to_idx, dataset_dir)

def create_symlinks_for_split(image_groups, image_list, split_name, class_to_idx, dataset_dir):
    """Create symlinks to original images and generate YOLO labels for test split."""
    images_dir = os.path.join(dataset_dir, split_name, 'images')
    labels_dir = os.path.join(dataset_dir, split_name, 'labels')
    ...
    for idx, image_filename in enumerate(image_list):
        ...
        if os.path.exists(source_image_path):
            ...
            os.symlink(os.path.abspath(source_image_path), target_image_path)
        ...
        if image_filename in image_groups:
            ...
            with open(label_path, 'w') as f:
                for annotation in annotations:
                    ...
                    # For test images, scale the bbox coordinates
                    scale_factor = RESIZE_SIZE / max(image_width, image_height)
                    scaled_bbox = {
                        'x': bbox['x'] * scale_factor,
                        'y': bbox['y'] * scale_factor,
                        'width': bbox['width'] * scale_factor,
    ...
    print(f"✅ Created {total_images} symlinks for {split_name} split")
    return total_images
...
def main():
    ...
    print(f"   🧪 Test images (symlinks): {test_count}")
